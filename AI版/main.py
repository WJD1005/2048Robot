...
default_text = '关注星云质心谢谢喵'  # logo下方默认文本
# 方块样式，[背景色, 字体颜色, 字号]
block_style = {
    0: '#cdc1b4',
    2: ['#eee4da', '#776e65', 60],
...
def game_init():
    ...
    # 贪心算法
    greedy_button = Button(screen_width - padding - button_width * 3 - horizontal_margin * 2,
                           padding + info_height + vertical_margin, button_width, button_height,
                           button_background_color, '贪心算法', font, font_color_light)
    greedy_button.display(screen)
    button_dic['greedy_button'] = greedy_button  # 保存到字典
    # AI
    ai_button = Button(screen_width - padding - button_width * 2 - horizontal_margin,
                       padding + info_height + vertical_margin, button_width, button_height, button_background_color,
                       'AI', font, font_color_light)
    ai_button.display(screen)
    button_dic['ai_button'] = ai_button  # 保存到字典
    # 读取最好成绩
...
def game_start(screen):
    ...
    # 显示初始内容
    show_text(screen, default_text)  # 显示logo下方默认文本
    draw_map(screen)  # 画地图
...
def greedy_run(screen):
    ...
    if not judge():
        return
    show_text(screen, '贪心算法运行中……')  # logo下方文本
    while not get_out_flag:
        # 有效移动
        if move_list[greedy.greedy_one_step(map)]():
            step += 1
        ...
        for event in pygame.event.get():
            ...
            # 鼠标
                ...
                # 贪心算法按钮
                if button_dic['greedy_button'].click_check(event.pos):
                    get_out_flag = True
                    show_text(screen, default_text)  # 显示logo下方默认文本
                    break
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if button_dic['new_game_button'].click_check(event.pos):
                    ...
                    break
                # 贪心算法按钮
                elif button_dic['greedy_button'].click_check(event.pos):
                    greedy_run(screen)
